<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:78:"C:\wamp64\www\o2o25project\tp5\public/../application/admin\view\Hall\list.html";i:1530329174;s:77:"C:\wamp64\www\o2o25project\tp5\application\admin\view\AdminPublic\public.html";i:1530412825;}*/ ?>
<!DOCTYPE html>
<html lang="en">
<head>
<title>放映厅列表</title>
...
<div id="content">
 ...
 <div id="content-header">
  <div id="breadcrumb">
   ...
   <a href="/hall/list" class="current">放映厅管理</a><a href="/hall/list" class="current">放映厅列表</a><a href="javascript:history.go(-1)" class="current"><i class="icon-undo"></i>后退</a>
  </div>
 </div>
      ...
      <div id="DataTables_Table_0_wrapper" class="dataTables_wrapper" role="grid">
       <div class="">
        <div id="DataTables_Table_0_length" class="dataTables_length">
          <a href="/hall/add" class="btn btn-primary">添加放映厅</a>
        </div>
       </div>
       <table class="table table-bordered data-table dataTable" id="DataTables_Table_0">
		...
		<th class="ui-state-default" role="columnheader" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" aria-sort="ascending" aria-label="Rendering engine: activate to sort column descending" style="width: 81px;">
          ...
          <th class="ui-state-default" role="columnheader" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" aria-label="Platform(s): activate to sort column ascending" style="width: 256px;">
           <div class="DataTables_sort_wrapper">
            形状
            <span class="DataTables_sort_icon css_right ui-icon ui-icon-carat-2-n-s"></span>
           </div></th>
        ...
        <?php if(is_array($halls) || $halls instanceof \think\Collection || $halls instanceof \think\Paginator): if( count($halls)==0 ) : echo "" ;else: foreach($halls as $key=>$row): ?>
         <tr class="gradeA odd text-center">
          <td class="  sorting_1">
          	<input type="checkbox" value="<?php echo $row['id']; ?>">
          </td>
          <td class=""><?php echo $row['id']; ?></td>
          <td class="" title="<?php echo $row['hallname']; ?>"><?php echo $row['hallname']; ?></td>
          <td class=""><?php echo $row['hallnum']; ?></td>
          <td class="" title="<?php echo $row['hallLayout']; ?>"><?php echo mb_substr($row['hallLayout'],0,38,'utf-8'); ?>...</td>
          <td class="td-status"><?php if($row['status']==0): ?><span class="btn btn-mini btn-warning">已启用</span><?php else: ?><span class="btn btn-mini btn-inverse">已禁用</span><?php endif; ?></td>
          <td class="td-manage">
          	<?php if($row['status']==1): ?>
            ...
            <?php endif; ?>
            <a href="/hall/edit/id/<?php echo $row['id']; ?>" class="btn btn-mini btn-info" title="修改"><i class="icon icon-pencil"></i></a>
          ...
          <tr>
              ...
              <?php echo $halls->appends($request)->render(); ?>
           		</div>
          	</td>
          </tr>
          <tr>
          ...
          <div class="dataTables_filter" id="DataTables_Table_0_filter">
            <form action="">
              <label>放映厅名称: <input type="text" name="keywords" value="<?php echo $keywords; ?>" aria-controls="DataTables_Table_0"><input type="submit" class="btn btn-info" value="搜索"></label>
            </form>
          </div>
...
function start(id){
  ...
  layer.confirm('确认要启用吗？',function(index){
    $.get("/hall/change",{id:id,status:0},function(data){
      if(data){
        if(data['code']==0){
...
function stop(id){
  ...
  layer.confirm('确认要禁用吗？',function(index){
    $.get("/hall/change",{id:id,status:1},function(data){
      if(data){
        if(data['code']==0){
          layer.msg(data['msg'],{icon:2,time:3000});
        }else{
...
function del(id){
	...
	layer.confirm('确认要删除吗？',function(index){
		//ajax交互数据 传入id
		$.get("/hall/delete",{id:id},function(data){
			//判断传回的数据
			if(data){
        if(data['code']==0){
          layer.msg(data['msg'],{icon:2,time:3000});
...
$(".dels").click(function(){
	...
	$.get("/hall/dels",{arr:arr},function(data){
		if(data){
      if(data['code']==0){
        layer.msg(data['msg'],{icon:2,time:3000});
      }else{
