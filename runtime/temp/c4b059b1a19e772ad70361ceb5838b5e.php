<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:83:"C:\wamp64\www\o2o25project\tp5\public/../application/admin\view\Link\link_list.html";i:1530416937;s:77:"C:\wamp64\www\o2o25project\tp5\application\admin\view\AdminPublic\public.html";i:1530412825;}*/ ?>
<!DOCTYPE html>
<html lang="en">
<head>
<title>友情链接列表</title>
...
<div id="content">
 ...
 <div id="content-header">
  <div id="breadcrumb">
   ...
   <a href="/link/list" class="current">友情链接管理</a><a href="/link/list" class="current">友情链接列表</a><a href="javascript:history.go(-1)" class="current"><i class="icon-undo"></i>后退</a>
  </div>
 </div>
      ...
      <div id="DataTables_Table_0_wrapper" class="dataTables_wrapper" role="grid">
       <div class="">
        <div id="DataTables_Table_0_length" class="dataTables_length">
          <a href="/link/add" class="btn btn-primary">添加友情链接</a>
        </div>
       </div>
       <table class="table table-bordered data-table dataTable" id="DataTables_Table_0">
        <thead>
         <tr role="row">
          ...
          <th class="ui-state-default" role="columnheader" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" aria-sort="ascending" aria-label="Rendering engine: activate to sort column descending" style="width: 21px;">
           ...
           <th class="ui-state-default" role="columnheader" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" aria-sort="ascending" aria-label="Rendering engine: activate to sort column descending" style="width: 41px;">
           <div class="DataTables_sort_wrapper">
           链接类型
            <span class="DataTables_sort_icon css_right ui-icon ui-icon-triangle-1-n"></span>
           </div></th>
          <th class="ui-state-default" role="columnheader" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" aria-label="Browser: activate to sort column ascending" style="width: 60px;">
           <div class="DataTables_sort_wrapper">
           网站名称
            <span class="DataTables_sort_icon css_right ui-icon ui-icon-carat-2-n-s"></span>
           </div></th>

          <th class="ui-state-default" role="columnheader" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" aria-label="Platform(s): activate to sort column ascending" style="width: 160px;">
           <div class="DataTables_sort_wrapper">
            网站地址(或LOGON)
            <span class="DataTables_sort_icon css_right ui-icon ui-icon-carat-2-n-s"></span>
           </div></th>
           <th class="ui-state-default" role="columnheader" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" aria-label="Platform(s): activate to sort column ascending" style="width: 110px;">
           <div class="DataTables_sort_wrapper">
            网站简述
            <span class="DataTables_sort_icon css_right ui-icon ui-icon-carat-2-n-s"></span>
           </div></th>
           <th class="ui-state-default" role="columnheader" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" aria-label="Platform(s): activate to sort column ascending" style="width: 60px;">
           <div class="DataTables_sort_wrapper">
            添加时时间
            <span class="DataTables_sort_icon css_right ui-icon ui-icon-carat-2-n-s"></span>
           </div></th>
          <th class="ui-state-default" role="columnheader" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" aria-label="Platform(s): activate to sort column ascending" style="width: 46px;">
           <div class="DataTables_sort_wrapper">
            前台显示隐藏
            <span class="DataTables_sort_icon css_right ui-icon ui-icon-carat-2-n-s"></span>
           </div></th>
          <th class="ui-state-default" role="columnheader" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" aria-label="Engine version: activate to sort column ascending" style="width: 90px;">
           <div class="DataTables_sort_wrapper">
            操作
            <span class="DataTables_sort_icon css_right ui-icon ui-icon-carat-2-n-s"></span>
           </div></th>
        ...
        <?php if(is_array($data) || $data instanceof \think\Collection || $data instanceof \think\Paginator): if( count($data)==0 ) : echo "" ;else: foreach($data as $key=>$row): ?>
         <tr class="gradeA odd text-center">
          ...
          <td class=""><input type="text" name="lsort" cid="<?php echo $row['id']; ?>" value="<?php echo $row['lsort']; ?>" style="width:15px;padding-left:11px"></td>
          <?php if($row['k_type']==0): ?><td class="">文字链接</td><?php else: ?> <td class="">LOGO链接</td><?php endif; ?>
          <td class=""><?php echo $row['title']; ?></td>
          <?php if($row['k_type']==0): ?><td class=""><a href="<?php echo $row['url']; ?>"><?php echo $row['url']; ?></a></td><?php else: ?> <td class=""><a href="<?php echo $row['url']; ?>"><img src="<?php echo $row['k_pic']; ?>" alt=""></a></td> <?php endif; ?>
          <td class="" title="<?php echo $row['desc']; ?>"><?php echo mb_substr($row['desc'],0,10,'utf-8'); ?></td>
          <td class=""><?php echo date('Y-m-d',(isset($data['k_addtime']) && ($data['k_addtime'] !== '')?$data['k_addtime']:time())); ?></td>
          <td class="td-status"><?php if($row['display']==0): ?><span class="btn btn-mini btn-warning">显示</span><?php else: ?><span class="btn btn-mini btn-inverse">隐藏</span><?php endif; ?></td>

          <td class="td-manage">
          	<?php if($row['display']==1): ?>
            <a href="/link/change/id/<?php echo $row['id']; ?>/display/0" onClick="return confirm('你确定上架吗？')" class="btn btn-mini btn-warning" title="上架">上架</a>
            <?php else: ?>
            <a href="/link/change/id/<?php echo $row['id']; ?>/display/1" onClick="return confirm('你确定下架吗？')" class="btn btn-mini btn-inverse" title="下架">下架</a>
            <?php endif; ?>
            <a href="/link/edit/id/<?php echo $row['id']; ?>" class="btn btn-mini btn-info" title="编辑">编辑</a>
          ...
          <?php if($res!=null): ?>
            <form action="">
              <label>链接类型:<select name="keywords1" value="<?php echo $k1; ?>" >
                                <option value="" selected="selected">全部链接</option>
                                <option value="0">文字链接</option>
                                <option value="1">LOGO链接</option>
                              </select>
              <input type="text" name="keywords" value="<?php echo $k; ?>" aria-controls="DataTables_Table_0" placeholder="网站名称关键字">
              <input type="submit" class="btn btn-info" value="搜索"></label>
            </form>
          <?php else: ?>
          暂无数据
          <?php endif; ?>
...
function del(id){
	...
	layer.confirm('确认要删除吗？',function(index){
		//ajax交互数据 传入id
		$.get("/link/delete",{id:id},function(data){
			//判断传回的数据
			if(data==1){
				if(data['code']==0){
...
$(".dels").click(function(){
	...
	layer.confirm('确认要删除吗',function(index){
	    $.get("/link/dels",{arr:arr},function(data){
	      if(data){
	        if(data['code']==0){
	          layer.msg(data['msg'],{icon:2,time:3000});
	        }else{
...
//修改排序
$("input[name='lsort']").blur(function(){
  lsort = $(this).val();
  id = $(this).attr("cid");
  $.get("/link/lsort",{id:id,lsort:lsort},function(data){
      //判断传回的数据
      if(data){
        if(data['code']==0){
          layer.msg(data['msg'],{icon:2,time:3000});
        }else{
          layer.msg('修改排序成功!',{icon: 1,time:2000});
        }
      }else{
        layer.msg('没有做任何修改!',{icon: 2,time:3000});
      }
    },'json');
})
</script>
