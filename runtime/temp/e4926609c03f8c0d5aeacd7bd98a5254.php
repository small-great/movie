<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:79:"C:\wamp64\www\o2o25project\tp5\public/../application/admin\view\Order\list.html";i:1530504446;s:77:"C:\wamp64\www\o2o25project\tp5\application\admin\view\AdminPublic\public.html";i:1530412825;}*/ ?>
<!DOCTYPE html>
<html lang="en">
<head>
<title>场次列表</title>
...
<div id="sidebar"><a href="#" class="visible-phone"><i class="icon icon-home"></i> 控制台</a>
  <ul>
    ...
    <li class="submenu open"> <a href="#"><i class="icon icon-reorder"></i><span>订单管理</span> </a>
...
<div id="content">
 ...
 <div id="content-header">
  <div id="breadcrumb">
   ...
   <a href="/order/list" class="current">订单管理</a><a href="/order/list" class="current">订单列表</a><a href="javascript:history.go(-1)" class="current"><i class="icon-undo"></i>后退</a>
  </div>
 </div>
    ...
    <div class="widget-box">
     ...
     <div class="widget-content nopadding">
      <div id="DataTables_Table_0_wrapper" class="dataTables_wrapper" role="grid">
        <div id="DataTables_Table_0_length" class="dataTables_length" style="margin-top:13px;margin-left:25px">
          <form action="">
              <label><input type="text" name="k" value="<?php echo $k; ?>" placeholder="订单号/用户名" style="width:200px !important"><input type="submit" class="btn btn-info" value="搜索" style="position:relative;top:-6px"></label>
            </form>
        </div>
       <table class="table table-bordered data-table dataTable" id="DataTables_Table_0">
        <thead>
         <tr role="row">
          ...
          <th class="ui-state-default" role="columnheader" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" aria-sort="ascending" aria-label="Rendering engine: activate to sort column descending" style="width: 106px;">
           <div class="DataTables_sort_wrapper">
            订单号
            <span class="DataTables_sort_icon css_right ui-icon ui-icon-triangle-1-n"></span>
           </div></th>
          ...
          <th class="ui-state-default" role="columnheader" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" aria-label="Platform(s): activate to sort column ascending" style="width: 206px;">
           <div class="DataTables_sort_wrapper">
            影院
            <span class="DataTables_sort_icon css_right ui-icon ui-icon-carat-2-n-s"></span>
           </div></th>
          <th class="ui-state-default" role="columnheader" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" aria-label="Platform(s): activate to sort column ascending" style="width: 156px;">
           <div class="DataTables_sort_wrapper">
            座位
            <span class="DataTables_sort_icon css_right ui-icon ui-icon-carat-2-n-s"></span>
           </div></th>
          <th class="ui-state-default" role="columnheader" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" aria-label="Platform(s): activate to sort column ascending" style="width: 50px;">
           <div class="DataTables_sort_wrapper">
            数量
            <span class="DataTables_sort_icon css_right ui-icon ui-icon-carat-2-n-s"></span>
            </div></th>
        ...
        <?php if(is_array($order) || $order instanceof \think\Collection || $order instanceof \think\Paginator): if( count($order)==0 ) : echo "" ;else: foreach($order as $key=>$row): ?>
         <tr class="gradeA odd text-center">
          <td class="  sorting_1">
          	<input type="checkbox" value="<?php echo $row['id']; ?>">
          </td>
          <td class=""><?php echo $row['orderid']; ?></td>
          <td class=""><?php echo $row['uname']; ?></td>
          <td class=""><?php echo $row['relss']; ?></td>
          <td class=""><?php echo $row['mname']; ?></td>
          <td class=""><?php echo $row['cname']; ?></td>
          <td class=""><?php echo $row['seat']; ?></td>
          <td class=""><?php echo $row['num']; ?></td>
          <td class=""><?php echo $row['price']; ?></td>
          <td><?php if($row['status']==0): ?>已支付(已取票)<?php elseif($row['status']==1): ?>已支付(已取票)<?php elseif($row['status']==2): ?>未支付<?php else: ?>已退票<?php endif; ?></td>

          <td class="td-manage">

            <a href="/order/edit/id/<?php echo $row['id']; ?>" class="btn btn-mini btn-info" title="修改"><i class="icon icon-pencil"></i></a>
...
$(".dels").click(function(){
	...
	$.get("/order/delete",{arr:arr},function(data){
    console.log(data);
		if(data){
      if(data['code']==0){
        layer.msg(data['msg'],{icon:2,time:3000});
