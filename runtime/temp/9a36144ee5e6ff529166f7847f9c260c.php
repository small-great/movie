<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:79:"C:\wamp64\www\o2o25project\tp5\public/../application/admin\view\Relss\list.html";i:1530365064;s:77:"C:\wamp64\www\o2o25project\tp5\application\admin\view\AdminPublic\public.html";i:1530412825;}*/ ?>
<!DOCTYPE html>
<html lang="en">
<head>
<title>场次列表</title>
...
<div id="content">
 ...
 <div id="content-header">
  <div id="breadcrumb">
   ...
   <a href="/relss/list" class="current">场次管理</a><a href="/relss/list" class="current">场次列表</a><a href="javascript:history.go(-1)" class="current"><i class="icon-undo"></i>后退</a>
  </div>
 </div>l
      ...
      <div id="DataTables_Table_0_wrapper" class="dataTables_wrapper" role="grid">
       <div class="">
        <div id="DataTables_Table_0_length" class="dataTables_length">
          <a href="/relss/add" class="btn btn-primary">添加场次</a>
        </div>
       </div>
       <table class="table table-bordered data-table dataTable" id="DataTables_Table_0">
        ...
        <?php if(is_array($relss) || $relss instanceof \think\Collection || $relss instanceof \think\Paginator): if( count($relss)==0 ) : echo "" ;else: foreach($relss as $key=>$row): ?>
         <tr class="gradeA odd text-center">
          <td class="  sorting_1">
          	<input type="checkbox" value="<?php echo $row['id']; ?>">
          </td>
          <td class=""><?php echo $row['id']; ?></td>
          <td class=""><?php echo $row['day']; ?></td>
          <td class=""><?php echo $row['start_time']; ?>-<?php echo $row['end_time']; ?></td>
          <td class=""><?php echo $row['cname']; ?></td>
          <td class=""><?php echo $row['mname']; ?></td>
          <td class=""><?php echo $row['hname']; ?></td>

          <td class="td-manage">

            <a href="/relss/edit/id/<?php echo $row['id']; ?>" class="btn btn-mini btn-info" title="修改"><i class="icon icon-pencil"></i></a>
          ...
          <tr>
              ...
              <?php echo $relss->appends($request)->render(); ?>
           		</div>
          	</td>
          </tr>
          <tr>
          ...
          <div class="dataTables_filter" id="DataTables_Table_0_filter">
            <form action="">
              <label>场次时间: <input type="text" name="time" value="<?php echo $time; ?>" onclick="WdatePicker({skin:'whyGreen',dateFmt:'H:mm'})" class="Wdate"><input type="submit" class="btn btn-info" value="搜索"></label>
            </form>
          </div>
...
function del(id){
  ...
  //console.log(ss);
	...
	layer.confirm('确认要删除吗？',function(index){
		//ajax交互数据 传入id
		$.get("/relss/delete",{id:id},function(data){
			//判断传回的数据
			if(data){
        if(data['code']==0){
...
$(".dels").click(function(){
	...
	$.get("/relss/dels",{arr:arr},function(data){
		if(data){
      if(data['code']==0){
        layer.msg(data['msg'],{icon:2,time:3000});
      }else{
